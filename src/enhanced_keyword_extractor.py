    ...
    def __init__(self, llm_config: Dict = None):
        ...
        Args:
            llm_config: Optional LLM configuration for hot topic analysis
                {'enabled': True, 'provider': 'deepseek', 'api_key': '...', 'model': 'deepseek-chat'}
        """
        ...
        if self.llm_config.get('enabled') and HAS_OPENAI:
            ...
            if api_key:
                if provider == 'deepseek':
                    ...
                    self.llm_model = self.llm_config.get('model', 'deepseek-chat')
                    logger.info(f"✅ DeepSeek LLM hot topic analysis enabled (model: {self.llm_model})")
                else:
                    # OpenAI or compatible
                    self.llm_client = openai.OpenAI(api_key=api_key)
                    self.llm_model = self.llm_config.get('model', 'gpt-4o-mini')
                    logger.info(f"✅ OpenAI LLM hot topic analysis enabled (model: {self.llm_model})")
            else:
